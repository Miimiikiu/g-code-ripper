...
print('Copyright (C) <2013-2021>  <Scorch>')
version = '0.22 Miimiikiu Custom'
...
if VERSION == 3:
    ...
    from tkinter.filedialog import * # analysis:ignore
    import tkinter.messagebox
else:
    from Tkinter import * # analysis:ignore
    from tkFileDialog import * # analysis:ignore
    import tkMessageBox
...
from math import * # analysis:ignore
import os
import re
import binascii # analysis:ignore
import getopt
import webbrowser
import struct
import shutil
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.probe_feed.set("20")
        self.probe_depth.set("-1")
        self.probe_safe.set("5")
        self.probe_nX.set("3")
        self.probe_nY.set("3")
        ...
        self.probe_soft.set("MACH3") # "LinuxCNC", "MACH3", "MACH4" or "DDCS"
        ...
        self.MAXZ    = 0
        self.MINZ    = 0
        self.HOME_DIR     =  os.path.expanduser("G:/My Drive/NC Files")
...
################################################################################
    ...
    def WriteGCode(self,side="base",Rstock=0.0,Wrap="XYZ"):
        ...
        if (side == "probe_n_cut") or (side == "probe_only") or (side =="probe_adjusted"):
            print('writing g code for probe_adjusted')
            gcode1 = []
            gcode2 = []
            ...
            if (side == "probe_n_cut") or (side == "probe_adjusted"):
                print('generating probe_n_cut/probe_adjusted g code')
    ...
    def Quit_Click(self, event): 
        print('Quit_Click')
        #os.remove('./gcr_temp/coords_temp.bin')
        #os.remove('./gcr_temp/probe_code_temp.bin')
    ...
    def menu_File_Open_G_Code_File(self):
        ...
        fileselect = askopenfilename(filetypes=[("G-code Files","*"),\
                                                ("All Files","*")],\
                                                 initialdir=init_dir)
    ...
    def menu_File_Read_Probe_data(self,junk=""):
        ...
        if fileselect != '' and fileselect != ():
            ...
            print('calling File_Read_Probe_data')
            self.File_Read_Probe_data(fileselect)
        print('end of menu_File_Read_Probe_data')
        print('saving adjusted g code file')
        #self.menu_File_Save_G_Code_Adjusted()
    ...
    def File_Save_G_Code_File_Auto(self,side="base",Rstock=0.0,Wrap="XYZ"):
        print('Saving G_Code File')
        if (self.Check_All_Variables() > 0):
            return
        ...
        for obj in gc.get_objects():
            obj_type = type(obj)
            if obj_type not in object_memory_usage:
                object_memory_usage[obj_type] = 0
            object_memory_usage[obj_type] += sys.getsizeof(obj)
        sorted_objects = sorted(object_memory_usage.items(), key=lambda x: x[1], reverse=True)
    ...
    def Master_Configure(self, event, update=0, Plot_Flag=True):
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            if self.gcode_op.get() == "split":
                ...
                print('refreshing probe operation')
...
##            self.Plot_Line_ISO(  minx,miny,minz,minx,miny,maxz, midH,midV,cszw,cszh,PlotScale,color,radius=rad)
        ...
        # THIS is where .coords is used
        
        if DEDICATED_FILE == True:
            with open('./gcr_temp/coords_temp.bin', 'rb') as infile:
                i = 0
                ...
                while True:
                    ...
                    mvtype = struct.unpack('<b', binary_data)[0] 
    
                    if mvtype == -1:
                        '''
                        binary_data = infile.read(2)
                        binary_data_length = struct.unpack('<H', binary_data)[0] #TODO: get length directly from string binary
                        print('length: {}'.format(binary_data_length))
                        binary_data = infile.read(binary_data_length)
                        print('binary data:{}'.format(binary_data))
                        line = struct.unpack('<{}s'.format(binary_data_length), binary_data)
                        #For comments, this comes out as something like b';('
                        
                        
                        print('full line:{}'.format(line))
                        line = line[0].decode('utf-8')
                        '''
                        print('NO WRITING AVAILABLE')
                    elif mvtype == 0:
                        '''
                        binary_data = infile.read(24)
                        line = struct.unpack('<ffffff', binary_data)
                        line = [mvtype, [line[0], line[1], line[2]], [line[3], line[4], line[5]]]
                        '''
                        print('NO WRITING AVAILABLE')
                        
                    elif mvtype == 1:
                        binary_data = infile.read(26)
                        line = struct.unpack('<ffffffbb', binary_data)
                        line = [line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7]]
                        
                    elif mvtype == 2 or mvtype == 3:
                        binary_data = infile.read(34)
                        line = struct.unpack('<ffffffbbff', binary_data)
                        line = [line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7], line[8], line[9]]
                    elif mvtype == 4: #Custom mvtype for this situation
                        binary_data = infile.read(26)
                        line = struct.unpack('<ffffffbb', binary_data)
                        line = [line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7]]
                        ...
                        try:
                            print('Warning: Offset! {}'.format(line))
                        except:
                            print('Warning: Offset! Movetype:{}'.format(mvtype))
                        #time.sleep(1)
                    
                    
                    #line = struct.unpack('<ffffffbb', binary_data)
                    if i % 1000000 == 0:
                    #print('plotting line {}/{}: {}'.format(i, len(self.coords), line))
                        #print('plotting line {}: {}'.format(i, line))
                        print('plotting mvtype {}: {}'.format(i, mvtype))
                        print('segID size: {}'.format(sys.getsizeof(self.segID))) # About 51 MB for test file
                        print('PreviewCanvas size: {}'.format(sys.getsizeof(self.PreviewCanvas)))
                    #for item in line:
    ...
    def DoIt(self):
        ...
        if self.gcode_op.get() == "split":
            ...
            for line in self.g_rip.left_side:
                if line[0] == 3 or line[0] == 2 or line[0] == 1:
                    ...
                    z2 = line[2][2]
                if line[0] == 1:
                    self.append([x1, y1, z1, x2, y2, z2, 2, line[0] ])
            ...
            print('probe_code')
            print('nX:{} nY:{} xPartitionLength:{} yPartitionLength:{}'.format(nX, nY, xPartitionLength, yPartitionLength))
            ...
            if DEDICATED_FILE == True:
                with open('./gcr_temp/coords_temp.bin', 'ab') as coords: #write binary to file for memory efficiency
                    print('opened coords_temp.bin')
                    with open('./gcr_temp/probe_gcode.bin', 'rb') as infile:
                    #for line_num, line in enumerate(self.g_rip.probe_gcode): 
                        
                        #if line_num % 100000 == 0:
                        #print('g_rip line {}/{}: {}'.format(line_num, len(self.g_rip.probe_gcode), line))
                        ...
                        while True:
                            ...
                            mvtype = struct.unpack('<b', binary_data)[0] # Unpacks into tuple # Expected to be an integer, (unless passthru)?
                            #print('unpacked mvtype: {} type: {}'.format(mvtype, type(mvtype)))
                            if mvtype == -1:
                                binary_data = infile.read(2)
                                binary_data_length = struct.unpack('<H', binary_data)[0] #TODO: get length directly from string binary
                                #print('length: {}'.format(binary_data_length))
                                ...
                                binary_data = infile.read(58) #12 + 8 + 20 + 8 + 8 + 2
                                line = struct.unpack('<fffhhhhfffffhhhhffH', binary_data)
                                line = [mvtype, [line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7], line[8]], 
                                        [line[9], line[10], line[11], line[12], line[13], line[14], line[15], line[16], line[17]], 
                                        line[18]]
                                
                            elif mvtype == 2 or mvtype == 3:
                                binary_data = infile.read(70) # 12 + 8 + 20 + 8 + 20 + 2
                                line = struct.unpack('<fffhhhhfffffhhhhfffffH', binary_data)
                                line = [mvtype, [line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7], line[8]], 
                                        [line[9], line[10], line[11], line[12], line[13], line[14], line[15], line[16], line[17]], 
                            ...
                            if line[0] == 3 or line[0] == 2 or line[0] == 1:
                                ...
                                z2 = line[2][2]
                                '''
                                if isinstance(x1, complex) or isinstance(y1, complex) or isinstance(z1, complex):
                                    x1 = 0
                                    y1 = 0
                                    z1 = 15 #safe height
                                    line[1][0] = 0
                                ...
                                '''
                            if line[0] == 1: #G1
                                coords.write(struct.pack('<bffffffbb', mvtype, x1, y1, z1, x2, y2, z2, 1, line[0] )) #mvtype to beginniong of coords
                                #coords.write(b'\n')
                            elif line[0] == 3 or line[0] == 2: #G2 or G3 (circular)
                                xc = line[3][0] #center
                                yc = line[3][1]
                                #print('xc:{} type:{}'.format(xc, type(xc)))
                                #print('yc:{} type:{}'.format(yc, type(yc)))
                                coords.write(struct.pack('<bffffffbbff', mvtype, x1, y1, z1, x2, y2, z2, 1, line[0], xc, yc  )) 
                                #coords.write(b'\n')
                            i += 1
                    ...
                    if (self.probe_data==[]):
                        for line in self.g_rip.probe_coords:
                            ...
                            if (line[0]):
                                ...
                                coords.write(struct.pack('<bffffffbb', line[0], xp-Lpt, yp    , zp, xp+Lpt, yp    , zp, 3, 1))
                                coords.write(struct.pack('<bffffffbb', line[0], xp    , yp+Lpt, zp, xp    , yp-Lpt, zp, 3, 1)) 
                                coords.write(struct.pack('<bffffffbb', line[0], xp    , yp    ,  0, xp    , yp    , zp, 3, 1))
                            else:

                                coords.write(struct.pack('<bffffffbb', line[0], xp    , yp    ,  0, xp    , yp    , zp, 3, 1))
                                coords.write(struct.pack('<bffffffbb', line[0], xp    , yp+Lpt, zp, xp    , yp-Lpt, zp, 2, 1))
                                coords.write(struct.pack('<bffffffbb', line[0], xp    , yp    ,  0, xp    , yp    , zp, 2, 1))
                    else:
                        mvtype = 4 #Custom mvtype for this situation
                        for line in self.probe_data:
                            ...
                            zp  = line[2] - Zoffset
                            Lpt= xPartitionLength/5
                            #print('probe_data line:{}'.format(line))
                            
                            coords.write(struct.pack('<bffffffbb', mvtype, xp-Lpt, yp    , zp, xp+Lpt, yp    , zp, 2, 1 ))
                            #coords.write(b'\n')
                            coords.write(struct.pack('<bffffffbb', mvtype, xp    , yp+Lpt, zp, xp    , yp-Lpt, zp, 2, 1 ))
                            #coords.write(b'\n')
                            coords.write(struct.pack('<bffffffbb', mvtype ,xp    , yp    ,  0.0, xp    , yp    , zp, 2, 1 ))
                            #coords.write(b'\n')
                print('closed coords_temp.bin')
            if DEDICATED_FILE == False:
        ...
        print('Ignoring Plot_Data()') # speed things up or break everything
        #print('Calling Plot_Data from DoIt')
        #self.Plot_Data()
        print('End DoIt')
        ################
        #   End DoIt   #
...
class G_Code_Rip:
    ...
    def Read_G_Code(self,filename, XYarc2line = False, arc_angle=10, units="in", Accuracy=""):
        ...
        for line_number, line in enumerate(fin):
            ...
            for code in ALL:
                ...
                while index != -1:
                    if line_number < 40:
                        #print('code found: {}'.format([code, index]))
                        pass
                    temp.append([code,index])
                    index = line.find(code,index+1)
            
            #for tempnum, item in enumerate(temp):
            #    if tempnum < 50:
            #        print('temp item: {}'.format(item))
            #        print(tempnum)
            if line_number < 40:
                #print('temp: {}'.format(temp))
                pass
            temp.sort(key=lambda a:a[1]) #Put the line into the original order
            if line_number < 40:
                #print('lambda temp: {}'.format(temp))
                pass
            ...
            if temp != []:
                ...
                while x <= len(temp)-1:
                    ...
                    x = x + 1
            #if line_number < 40:
                #print('code_line: {}'.format(code_line))
            #################################
                    
            mv_flag   = 0
            POS_LAST = POS[:] #STARTS AT COMPLEX
            #CENTER  = ['','','']
            CENTER   = POS_LAST[:]
            passthru = ""
            ...
            #for com in code_line:
                ...
                if com[0] == "G":
                    ...
                    if Gnum == "0" or Gnum == "1":
                        ...
                        plane = Gnum
                        # passthru is not necessary since xind etc accounts for this.
                    elif Gnum == "18":
                        plane = Gnum
                    elif Gnum == "19":
                        plane = Gnum
                        ...
                        # G90 & G91 may not be necessary... It seems like the elif com[0] == 'X' part 
                        # suggests that it's always absolute and if necessary accounts for incremental moves.
        ...
        while ((cnt < len(self.g_code_data)) and (isinstance(first_sum, complex))):
            ...
            if cnt < 50:
                print('g code line: {}'.format(line)) #ambiguous because z0 + xj + yj is complex
            if line[0] == 0 or line[0] == 1 or line[0] == 2 or line[0] == 3:
                ...
                print('firstx: {} firsty: {} first z:{}'.format(firstx, firsty, firstz))
            cnt=cnt+1
            first_sum = firstx + firsty + firstz
        max_cnt = cnt
        cnt = 0
        ...
        while (cnt < max_cnt):
            ...
            if line[0] == 1 or line[0] == 2 or line[0] == 3:
                ...
                if (isinstance(line[1][0], complex)):
                    self.g_code_data[cnt][1][0] = firstx
                if (isinstance(line[1][1], complex)):
                    self.g_code_data[cnt][1][1] = firsty
                if (isinstance(line[1][2], complex)):
                    self.g_code_data[cnt][1][2] = firstz
            cnt=cnt+1
        if (ambiguousX or ambiguousY or ambiguousZ):
            MSG = "Ambiguous G-Code start location:\n"
            # G1 accounts for acceleration using current position while G0 does not
            ...
            READ_MSG.append(MSG)
        #for num, item in enumerate(self.g_code_data):
            #if num < 40:
                #print('g_code_data: {}'.format(item))
    ...
    def convert_bytes_to_list(self, outfile):
        pass
    ...
    #def probe_code(self,code2probe,nX,nY,probe_istep,minx,miny,xPartitionLength,yPartitionLength,Xoffset,Yoffset,Zoffset):
        ...
        if DEDICATED_FILE == True:
            with open('./gcr_temp/probe_code_temp.bin', 'wb') as outfile:
                print('probe code: dedicated file')
                ...
                for i, line in enumerate(code2probe):
                    #if i % 100000 == 0:
                    #if i < 50:
                        #print('code2probe line {}/{}: {}'.format(i, len(code2probe), line))
                    if line[0] == 0  or line[0] == 1:
                        ...
                        POS_LAST = line[1][:]
                        POS      = line[2][:]
                        CENTER   = line[3][:] # Center of arc
                        feed     = line[4]
                        for item in CENTER:
                            #print('CENTER:{}'.format(CENTER))
                            #print('code2probe line {}/{}: {}'.format(i+1, len(code2probe), line))
                    ...
                    if mvtype >= 0 and mvtype <=3:
                        ...
                        if mvtype == 0:
                            ...
                            outdata = struct.pack('<bffffff', mvtype, *pos_last, *pos)
                            outfile.write(outdata)
                            #if i < 50:
                            #    print('movetype 0: {} {} {} {} {} {}'.format(*pos_last, *pos))
                            #    print('outdata:{}'.format(struct.unpack('<bffffff', outdata)))
                            #if i < 50:
                            #    print('movetype 0: {} {} {} {} {} {}'.format(*pos_last, *pos))
                        ...
                        if mvtype == 1:
                            ...
                            if (length <= min_length):
                                ...
                                outdata = struct.pack('<bffffffH', mvtype, *pos_last, *pos, int(feed))
                                #if i < 50:
                                #    print('UNDER MINIMUM LENGTH')
                                #    print('movetype 1: {} {} {} {} {} {} {}'.format(*pos_last, *pos, int(feed)))
                                #    print('outdata:{}'.format(struct.unpack('<bffffffH', outdata)))
                                ...
                                for n in range(1,Lsteps+1):
                                    ...
                                    outdata = struct.pack('<bffffffH', mvtype,xstp0,ystp0,zstp0,xstp1,ystp1,zstp1,int(feed))
                                    outfile.write(outdata)
                                    #if i < 50:
                                    #    print('movetype 1: {} {} {} {} {} {} {}'.format(xstp0,ystp0,zstp0,xstp1,ystp1,zstp1,int(feed)))
                                    #    print('outdata: {}'.format(struct.unpack('<bffffffH', outdata)))
                                    xstp0 = float(xstp1)
                                    ystp0 = float(ystp1)
                                    zstp0 = float(zstp1)
                        ...
                        if mvtype == 2 or mvtype == 3:
                            ...
                            outfile.write(struct.pack('<bfffffffffH', mvtype, *pos_last, *pos, *center, int(feed))) 
                    ###############################################################################
                    else:
                        if passthru != '':
                            #print('passthru: {}'.format(passthru))
                            if passthru[0] == ';':
                                #out.append(passthru) # Used for G28, comments, etc.
                                outfile.write(struct.pack('<b', -1))
                                outfile.write(struct.pack('<H', len(passthru[1]) + 1)) # ; only for comments, not special codes
                                outfile.write(struct.pack('<{}s'.format(len(passthru[1]) + 1), ';'.encode('utf-8') + passthru[1].encode('utf-8')))
                            else:
                                outfile.write(struct.pack('<b', -1))
                                outfile.write(struct.pack('<H', len(passthru) - 1)) 
                                outfile.write(struct.pack('<{}s'.format(len(passthru) - 1), passthru.encode('utf-8')))
                            
                            
            with open('./gcr_temp/probe_code_temp.bin', 'rb') as infile:
                
                with open('./gcr_temp/probe_gcode.bin', 'wb') as outfile:
                    ################################
                    ##  Loop through output to    ##
                    ##  find needed probe points  ##
                    ################################
                    print('Searching for probe points')
                    ...
                    while True:
                        ...
                        if mvtype == -1:
                            binary_data = infile.read(2)
                            binary_data_length = struct.unpack('<H', binary_data)[0] #TODO: get length directly from string binary
                            #('length: {}'.format(binary_data_length))
                            binary_data = infile.read(binary_data_length)
                            #print('binary data:{}'.format(binary_data))
                            line = struct.unpack('<{}s'.format(binary_data_length), binary_data)
                            ...
                            binary_data = infile.read(26)
                            line = struct.unpack('<ffffffH', binary_data)
                            line = [mvtype, [line[0], line[1], line[2]], [line[3], line[4], line[5]], line[6]]
                            
                        elif mvtype == 2 or mvtype == 3:
                            binary_data = infile.read(38)
                            line = struct.unpack('<fffffffffH', binary_data)
                            line = [mvtype, [line[0], line[1], line[2]], [line[3], line[4], line[5]], [line[6], line[7], line[8]], line[9]]
                        else:
                            #binary_data = infile.read()
                            print('Warning: Offset! {}'.format(line))
                            break
                            quit()
                        #print('unpacked data: {}'.format(line))
                        ...
                        if mvtype >= 1 and mvtype <=3:
                            ...
                            line[1].append(p_index_C+BPN)
                            line[1].append(p_index_D+BPN)
                            line[1].append(Xfraction)
                            line[1].append(Yfraction)
                            #print('Line[1] after: {}'.format(line[1]))
                            ...
                            line[2].append(Xfraction)
                            line[2].append(Yfraction)
                            #print('Line[2] after: {}'.format(line[2]))
                            try:
                                POINT_LIST[p_index_A ] = True
                                POINT_LIST[p_index_B ] = True
                                POINT_LIST[p_index_C ] = True
                                ...
                                pass
                        #self.probe_gcode.append(line)
                        #outfile.write(struct())
                        
                        #if i % 10000 == 0:
                        #    print('\ni: {}'.format(i))
                        #print('line:{}'.format(line))
                            #if self.probe_gcode_prev == self.probe_gcode:
                            #    print('SAME!')
                            #    print('prev:{}'.format(self.probe_gcode_prev[-1]))
                            #    print('current:{}'.format(self.probe_gcode[-1]))
                            #self.probe_gcode_prev = list(self.probe_gcode)
                        
                        if mvtype == 0:
                            #out.append( [mvtype,pos_last,pos] )
                            outdata = struct.pack('<bffffff', mvtype, *line[1], *line[2]) # No change
                            outfile.write(outdata)
                        if mvtype == 1:
                            outdata = struct.pack('<bfffhhhhfffffhhhhffH', mvtype, *line[1], *line[2], line[3])
                            outfile.write(outdata)
                        if mvtype == 2 or mvtype == 3:
                            outfile.write(struct.pack('<bfffhhhhfffffhhhhfffffH', mvtype, *line[1], *line[2], *line[3], line[4]))
                        ...
                        if mvtype == -1:
                            if passthru != '':
                                ...
                                if passthru[0] == ';':
                                    # comments
                                    outfile.write(struct.pack('<b', -1))
                                    outfile.write(struct.pack('<H', len(passthru))) # ; only for comments, not special codes
                                    outfile.write(struct.pack('<{}s'.format(len(passthru)), passthru.encode('utf-8')))
                                else:
                                    # G28, M2, etc.
                                    outfile.write(struct.pack('<b', -1))
                                    outfile.write(struct.pack('<H', len(passthru)))  # or maybe length - 1?
                                    outfile.write(struct.pack('<{}s'.format(len(passthru)), passthru.encode('utf-8')))
    ...
    def scale_translate(self,code2translate,translate=[0.0,0.0,0.0]):
        ...
        if translate[0]==0 and translate[1]==0 and translate[2]==0:
            print('returning code2translate')
            ...
            with open('./gcr_temp/probe_gcode.bin', 'rb') as infile:
                ...
                while True:
                    ...
                    if mvtype == -1:
                        ...
                        #print('full line 1st loop:{}'.format(line))
                        line = line[0].decode('utf-8')
                    elif mvtype == 0:
                        binary_data = infile.read(24)
                        line = struct.unpack('<ffffff', binary_data)
            ...
            with open('./gcr_temp/probe_gcode.bin', 'rb') as infile:
                ...
                while True:
                    ...
                    if mvtype == -1:
                        ...
                        line = line[0].decode('utf-8')
                        #print('full line 2nd loop:{}'.format(line))
                    elif mvtype == 0:
                        binary_data = infile.read(24)
                        line = struct.unpack('<ffffff', binary_data)
                        line = [mvtype, [line[0], line[1], line[2]], [line[3], line[4], line[5]]]
                    ...
                    if (line[0] == 1) or (line[0] == 2) or (line[0] == 3):
                        ...
                        if probe_data!=[]:
                            ...
                            if (not isinstance(coordB[2], complex)):
                                ...
                                #print('AXIS[2]:{}, First_Z_Safe:{}'.format(AXIS[2],First_Z_Safe))
                        ...
                        g_code.append(line)
                    if i % 100000 == 0:
                        print('g_code size: {}'.format(sys.getsizeof(g_code)))
                    i += 1
